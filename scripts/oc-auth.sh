...
if [[ ! "${POD}" ]]; then
    ...
    export CLOUDIGRADE_TOKEN=$(oc rsh -c "${CONTAINER_NAME}" "${POD}" scl enable rh-python36 -- python manage.py drf_create_token "${CLOUDIGRADE_USER}" | grep -e 'token \K\w+(?= for)' -oP)
