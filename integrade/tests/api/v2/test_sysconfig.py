...
def is_on_local_network():
    ...
    try:
        ...
        return False
>>>>>>> initial test of sysconfig for 3scale auth
...
=======
    ...
    creds = ('mpierce@redhat.com', 'redhat')
    V2_QA_URL="https://api.access.qa.cloud.paas.upshift.redhat.com/r/insights/platform/cloudigrade/api/v2/sysconfig/"
    qa_branch = '3scale-investigation'
    qa_url = V2_QA_URL
    test_headers = {'X-4Scale-Env': 'ci', 'X-4Scale-Branch': qa_branch}
    qa_response = requests.get(qa_url, auth=creds, headers=test_headers, verify=False)

    V2_STAGE_URL="https://api.access.stage.cloud.paas.upshift.redhat.com/r/insights/platform/cloudigrade/auth/"
    stage_url = V2_STAGE_URL
    stage_headers = {'X-4Scale-Env': 'qa'}
    stage_response = requests.get(stage_url, auth=creds, headers=stage_headers, verify=False)
>>>>>>> initial test of sysconfig for 3scale auth
